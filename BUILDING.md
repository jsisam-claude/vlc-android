...
`project(':libvlcjni:libvlc')`, `project(':medialibrary')` and
`project(':application:remote-access-client')` instead of prebuilt artifacts.
...
#    remoteaccess @ 2dcf214a, sqlite source archive (SHA-512 pinned)
...
- `compile-remoteaccess.sh` honors the vendored web-client source.
...
| npm registry | **only** if you build the optional remote-access web UI (`remoteaccess/` + `npm run build-android`); skipping it just leaves that screen empty |
